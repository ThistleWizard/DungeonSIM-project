...
TASK 6 - WORLD LOGIC: Movement and topology per <room_and_movement_protocol> (topology lock, room render needed?). Tick the turn counter, light sources, timed effects. Mob/NPC autonomous behavior consistent with their motives and the noise the player made.
...
TASK 10 - COMPOSE: List the narrative beats as terse fragments (5-10 words each), then END reasoning and write the response - prose composed here for the first and only time - in this exact order:
  ...
  (c) <UpdateDungeon> block (mutations only; emit an empty <UpdateDungeon></UpdateDungeon> if nothing changed),
