...
COMMAND GRAMMAR (one statement per line; paths are relative to the dungeon root - NO 'dungeon.' prefix):
  ...
  _.move('fromItemPath', 'toArrayPath'[, count]);//reason -> relocate an OBJECT between containers, state intact. Drop: _.move('inventory.<id>', 'rooms.R##.contents'). Pick up: _.move('rooms.R##.contents.<id>', 'inventory'). Use this (not remove+insert) so a lit torch keeps its live, script-tracked fuel. Pass a COUNT to take/drop PART of a stack: _.move('rooms.R##.contents.clay_jars', 'inventory', 1) takes ONE of several - the script decrements the source stack and merges into the destination for you (omit count = move the whole stack).
...
RULES (the script enforces these; a command that violates them is rejected and logged):
  ...
  2. MAP IS APPEND-ONLY & TOPOLOGY-LOCKED.
     ...
     - AMBIENT LIGHT: most rooms are DARK - the player needs a torch. If a room has its OWN natural light (a shaft of daylight, lava glow, luminous fungus, a perpetual flame on an altar), add `ambient_light:'<short description of that light>'` to the room-add, e.g. _.assign('rooms.R##', {id:'R##',name:'...',descr:'...',exits:{...},contents:[],visited:true,ambient_light:'a shaft of grey daylight from a cracked ceiling'}). The script then treats the room as lit with NO torch and shows your description on the Light line. Omit it (default) for ordinary dark rooms. Set it only at room creation; the rest of lighting is script-owned (rule 8).
  3. INVENTORY changes only via in-fiction events (take/drop/consume/break/gift): _.insert to gain a new item; _.remove by id (with a count for stacks) to lose or use up. Change a carried item's FIELDS in place by id: _.set('inventory.<id>.equipped', <old>, <new>), _.set('inventory.<id>.worn', ...), _.add('inventory.<id>.charges', -1). Quantities exact.
     - QUANTITY IS CONSERVED (every stack, in inventory AND in room contents). A stack's count changes ONLY when the fiction adds or removes that many. Whenever you pull a distinct item OUT of a stack - the player takes some, OR you open/examine a stack and reveal one of them as a specific item - you MUST subtract that many from the source stack in the SAME turn. NEVER add a derived item without decrementing its origin (that silently INFLATES the count - 3 jars become 4). To TAKE part of a stack, use _.move with a count (the script does the subtract+merge). To REVEAL one of a stack as something new, do BOTH a subtract AND an insert. WORKED EXAMPLE - a room holds clay_jars x3 and you decide one contains salt: emit _.add('rooms.R02.contents.clay_jars.qty', -1);//one jar held salt  AND  _.insert('rooms.R02.contents', {id:'jar_salt',name:'Jar of Coarse Salt',qty:1});//revealed - so 2 plain jars + 1 salt jar = the original 3, NOT 4. (If revealing empties the stack, _.remove it instead.)
  ...
  5. BESTIARY is append-only. The first time a mob TYPE appears, _.assign('bestiary.<type>', {sprite_fragment:'<8-15 word canonical visual>', hp_base:<n>, defense:<n>, tags:[<descriptors>]}). The `tags` drive the creature's SPRITE: the script matches them against a sprite library and LOCKS a picture to each mob, so choose them well - once per type. Pick from this vocabulary (free extra tags are allowed but only these are matched). ARCHETYPE - exactly one, listed FIRST: humanoid, beast, undead, construct, ooze, dragon, insectoid, demon, elemental, plant, vermin, aberration, fey. SIZE: tiny, small, medium, large, huge. TRAITS: armored, winged, horned, robed, skeletal, fanged, tentacled, fiery, icy, spectral, aquatic, clawed, scaled, furred, multi_eyed. E.g. a goblin -> tags:['humanoid','small','fanged']; a wraith -> tags:['undead','spectral','robed']; a fire beetle -> tags:['insectoid','small','fiery']. Never edit an existing entry.
  6. COMBAT: set combat.active true/false; manage combat.mobs as an array. SPAWN a mob with the FULL shape: _.insert('combat.mobs', {id:'goblin_1',type:'goblin',name:'Goblin Skirmisher',hp_cur:8,hp_max:8,status:'',pos:'near'}). Required fields: id, type, name (a STRING), hp_cur, hp_max, status (a STRING - one of '', 'bloodied', 'prone', 'unaware', etc.; NOT a list), pos ('near' or 'far'). Address a living mob's fields by id: _.add('combat.mobs.<id>.hp_cur', -3), _.set('combat.mobs.<id>.status', '', 'bloodied'). For an unalerted ambush target use status:'unaware' and clear it (status set back to '') when combat joins. Do NOT invent extra fields (e.g. `aware`) - they are dropped by the schema; put that info in `status`.
     - KILLING a mob: mark it dead - _.set('combat.mobs.<id>.status', '<old>', 'dead'). The script then DROPS A CORPSE into the player's current room and removes the mob from combat for you. Do NOT _.remove a kill and do NOT hand-author the body - the script owns it (this is why your goblin left no corpse before). To make a mob FLEE / vanish / be banished WITHOUT leaving a body, use _.remove('combat.mobs', '<id>') instead. End the fight by setting combat.active false when the last threat is gone.
     - LOOT IS REVEALED ON SEARCH, never at the kill: the auto-dropped corpse holds nothing until the player searches it. On the turn the player searches a body, decide what it carried and _.insert those objects into that room's contents (then they show in 'Here:' and can be taken). NEVER put a slain mob's loot straight into the player's inventory - it goes on the floor first (see rule 4).
...
Announce the character in a short system-voice summary (stats visible here, once). Render the first room of the dungeon per <room_and_movement_protocol> (R01). This threshold room is ALWAYS lit by its own ambient light - but the character's torches start UNLIT, so work a clear hint into the first room's narration that the dark beyond the threshold is total and they must strike a light before pressing deeper. Generate the player sprite per <sprite_protocol>. Then SEED the state in one <UpdateDungeon> block, using _.assign for objects and _.insert per inventory item (these skip the old-value check, correct for fresh state):
  ...
  _.assign('rooms.R01', {id:'R01',name:'<Room Name>',descr:'<short>',exits:{<dir>:{to:null,type:'<type>',state:'open'}},contents:[],visited:true,ambient_light:'<the light that fills this threshold, e.g. grey daylight spilling down the entry stair>'});//first room - ALWAYS lit via ambient_light (the threshold has its own light); each visible way out gets to:null until explored
...
PATCHES = [
    ...
    (
        ...
        "To kill a mob, mark it status:'dead' (see <mutation_protocol> rule 6): the script drops its corpse into the current room and removes it from combat. The corpse persists; its carried loot is revealed only when the player SEARCHES the body (then _.insert those items into the room's contents).",
    ),
    # [13] Event Table
    (
    ...
    # [11] Room & Movement — generation style now knows some rooms are self-lit.
    (
        "Seed environmental tools the player can exploit creatively (oil, braziers, chains, grates, loose masonry) - the world should reward lateral thinking.",
        "Seed environmental tools the player can exploit creatively (oil, braziers, chains, grates, loose masonry) - the world should reward lateral thinking. Most rooms are DARK (the player needs a torch); occasionally a room has its OWN light - a shaft of daylight, lava glow, luminous fungus, a lit shrine - give those `ambient_light` in the room-add (see <mutation_protocol>) so they read as lit without a torch.",
    ),
]
