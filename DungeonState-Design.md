...
**M6 automap — done & live-verified in SillyTavern:** Pure `renderMap()` in `src/map.ts` (deterministic + stable grid-walk, current-depth filter, current-room amber highlight, undiscovered `?` stubs, vertical/portal markers, `data-room-id`/`<title>` hooks) + exported `computeLayout` seam; `/map` slash command in `src/commands.ts` (injectable `registerMapCommand` + `bootstrapCommands` reading `SillyTavern.getContext()`), wired from runtime's bootstrap. Forward-compat schema fields landed (additive/defaulted, M1–M5 safe): exit `category`, true `lock`, `lock_revealed`; room `depth` — portals/branches/discoverable-locks are REPRESENTABLE; their gameplay stays deferred. The knowledge model is **cartographer-style** (a deliberate refinement of B6, confirmed with the user): the map shows what the player can SEE (the exit's fiction/`type`) but hides the WIRING (`category`) and destination until a link is traversed — so visible stairs read as `↓`/`↑` immediately (you can see the staircase in your own room), while a trapdoor disguised as an archway stays an ordinary unexplored stub until used. Secret (`state:'hidden'`) exits and unrevealed locks render nothing. All test-guarded; 86 tests pass. **Live-confirmed:** `/map` registers and renders an accurate current-depth automap in the running app.

**M8 prep — sheet + inventory renderers done & live-verified in SillyTavern:** `src/sheet.ts` (`renderSheet`, `renderInventory`) renders the full structured Gold Box character sheet (identity, HP bar, defense, light, six abilities, skills with rank-dots + marks-to-next, condition chips) and inventory (qty, equipped/worn/charges, notes) as styled HTML straight from `player.*` / `inventory[]` — the same anti-drift move as the automap (engine prints it, can't disagree with state). Wired as `/character` + `/inventory` via `registerAllCommands`/`bootstrapCommands`. Pulled forward so M8 collapses to layout/CSS over proven renderers. (`/spellbook` deferred: no spells/known-abilities field exists in the schema yet — that arrives with the §13 class-cartridge layer, and the renderer is trivial once it does.)

**M8 Gold Box display panel — done & live-verified in SillyTavern** (button + `/display`, per-turn updates, swipe/delete consistency, narrow→tabs responsiveness all confirmed). Open follow-up: a **graphical/appearance pass** — the renderers and wiring work, but the panel's look (layout, spacing, retro styling) is not yet dialed in; refine it visually (likely alongside the M7 sprite work, which fills the viewport). `src/display.ts`: pure `renderDisplay` assembles a 2×2 dashboard of four tiles (Viewport · Map · Character · Inventory), each reusing the existing renderers; `renderViewport` is a live text stand-in (current room, or faced mob in combat) carrying a `data-viewport` hook M7 fills with the sprite. Container-responsive (`@container`): wide panel → 2×2 quadrants, narrow dock/phone → single column + tab bar (the user confirmed: docked four-quadrant on desktop, tabs on mobile — "tabs" alone was a slip). `bootstrapDisplay` (guarded, ST-only) mounts a fixed right-rail into the parent ST DOM via `$(sel, window.parent.document)`, registers an extension-style **"Display" input-bar widget** (Tavern Helper `replaceScriptButtons` + `eventOnButton`) plus a `/display` toggle, persists on/off in localStorage, **default OFF** (§14 hard requirement; reflows `#sheld` only when on; full-screen + tabs on phones). Lives every turn AND every rewind: `createRuntime` gained an `onRefresh` listener registry fired inside `refreshInjection`; `bootstrap()` registers the panel's `refresh` there — so map/sheet can never show post-turn state while the chat shows a rewound turn (the §14 M5 dependency, satisfied for free). Shared `src/style.ts` (palette + `panel()` card) keeps the tiles identical; `sheet.ts` refactored onto it. 100 tests pass. (We took §14 build-path 1, in-ST overlay; path 2 external page stays deferred. Clickable-room/deep-interaction and drag/resize deferred.)

**Engine refinements (post-M8, landed alongside the panel; from live playtest).** Three coupled changes that came out of a torch/inventory bug: (1) **id-keyed array addressing** in the applier — the model can address any array element's field by id (`inventory.torch_1.lit`, `combat.mobs.<id>.hp_cur`); an unresolved id is blocked, never corrupts the tree. (2) **Unified `ItemSchema` + rooms-as-containers** — one object shape for `player.inventory` and a room's `contents`, so drop/pick-up is relocating an object between arrays; items carry `kind`/`equipped`/`worn`/`charges`/`lit`/`fuel`. The `move` verb (previously a blocked stub) now relocates an object between containers with state intact. (3) **Script-owned light economy** — a torch is an Item with `lit`+`fuel`; each turn the applier burns lit sources by the `meta.turn` delta, removes spent ones, and DERIVES the `light` block from whatever's lit in the pack/current room (model never tracks ticks). Fixes tick-drift and enables snuff/relight + lay-down-while-burning. Rooms-as-containers generalizes beyond torches: dropped gear, a corpse's loot, a disarmed trap all persist on the floor. Preset rewritten to match; 111 tests pass. (Unit-tested; live-test pending.)

**Next: M7 sprite system — LIBRARY-first selection, generation as a fallback (reframed with the user; supersedes the earlier generate-and-cache plan).** The cleaner move: don't make an unreliable image model reinvent a sprite every time — give the model a CONSTRAINED SELECTION over a curated, tagged sprite pack, the same division of labour as everywhere (model decides WHAT, the script computes/locks). The image model generating is the flaky part; picking from a fixed menu is not. **Ladder: curated library (primary) → image-gen (fallback) → text/placeholder (final).** This demotes the finicky thing (gen) to an edge case that only fires for something the pack doesn't cover.
- **Selection = tags; resolution = script.** When a new bestiary TYPE first appears the model emits descriptive tags (a structured form of the `sprite_fragment` it already writes, e.g. `["undead","humanoid","armored"]`). A PURE resolver matches tags → a concrete sprite deterministically, with `hash(id)` as the tie-break (so two goblins get `goblin_01` vs `goblin_02` — consistency AND variety), then writes the resolved sprite ref onto the bestiary entry, LOCKED thereafter. The model never sees the catalog (no token cost); the resolver is pure → **unit-testable**, unlike the gen path.
- **Pack-agnostic (decided with the user).** The pack is CONTENT, not engine: a manifest `{ id: { tags, src } }` living outside per-chat state (like the preset / §13 cartridges). New pack = drop in files + manifest, zero engine change; the resolver doesn't care where sprites come from. 8-bit sprites are tiny, so the manifest can inline them as base64 → fully self-contained, no asset-hosting.
- **Licensing (shipping caveat, not an architecture issue).** Actual Gold Box rips are copyrighted (D&D = WotC/Hasbro; sold DRM-free on GOG ≠ freely licensed) — fine for PERSONAL local play, NOT redistributable. So ship a CC0/properly-licensed retro pack as the default (DawnLike / Oryx / Kenney-class) for a clean shareable v1; pack-agnostic means anyone can drop in Gold Box rips locally for the authentic look. Same system, different manifest.
- **Gen fallback** keeps the cache-once design: a missing sprite is generated ONCE via ST's Image Generation extension (`triggerSlash('/sd quiet=true <fragment>')` — inherits the user's configured Source, zero backend code), cached by id; `hash(id)` is the seed/regeneration recipe (a seed alone only reproduces on the same backend, so the cache, not the seed, is the guarantee). Toggleable; off → text/placeholder; never blocks play.
- **Viewport wiring:** the M8 `data-viewport` hook just renders an `<img src=ref>` — library vs gen is invisible to the panel. **Location sprites deferred** (per-archetype backgrounds out of scope for v1).
...
- **Genre reskins (the engine is content-agnostic)** — because state/rules/sprites/preset are all decoupled CONTENT (pack-agnostic sprites + §13 cartridges + a swappable preset), DungeonState's engine isn't D&D-specific: a different preset + sprite pack + cartridges yields a different game on the same deterministic spine. Noted interest: a **Faith-style retro pixel-horror mode** (and similar). Far-future — the discipline is to ship the Gold Box core first; this is just logged so the architecture keeps the option open (it already does).
